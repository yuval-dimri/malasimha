...
def update_parameters():
    with st.sidebar:
        ...
        if search_query:
            filtered_params = {key: value for key, value in json_communicator.params.items(
        ...
        for key, value in filtered_params.items():
            new_value = st.text_input(f"{key}:", value=str(
                json_communicator.get_param(key)))
...
def main():
    ...
    num_boxes = st.number_input(
        "Number of Boxes:", min_value=1, max_value=10, value=3)
